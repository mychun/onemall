    ...
    <div class="orderDetail">
        ...
        <div class="order-goods">
            <div class="cgoods-info" v-for="item in checkedGoodsList" :key="item.id">
                <div class="cg-img"><img :src="item.picUrl" /></div>
                <div class="cg-wrap">
                    <h3>{{item.goodsName}}</h3>
                    <p>
                    <span class="cg-spec" v-for="(spec, index) in item.specifications" :key="index">{{spec}}</span>
                    </p>
                    <p class="row-b">
...
export default {
    ...
    methods: {
        init(){
            cartCheckout({cartId: 0, addressId: 0, couponId: 0, userCouponId: 0, grouponRulesId: 0}).then(res => {
